...
public class CursorLoader {
	 ...
	 * @throws LWJGLException Indicates a failure to create the hardware cursor
	 */
	public Cursor getCursor(ByteBuffer buf, int x, int y, int width, int height) throws LWJGLException {
		for (int i = 0; i < buf.limit(); i += 4) {
			byte red = buf.get(i);
			byte green = buf.get(i + 1);
			byte blue = buf.get(i + 2);
	 ...
	 * @throws LWJGLException Indicates a failure to create the hardware cursor
	 */
	public Cursor getCursor(ImageData imageData, int x, int y) throws LWJGLException {
