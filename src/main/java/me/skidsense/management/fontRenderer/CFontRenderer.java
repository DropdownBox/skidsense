...
public class CFontRenderer extends CFont {
    ...
    public float drawString(String text, double x, double y, int color, final boolean shadow) {
    	if(text.contains(Minecraft.getMinecraft().session.getUsername()) && Client.getModuleManager().getModuleByName("NameProtect").isEnabled()) {
    		text.replace(Minecraft.getMinecraft().thePlayer.getName(), "VicetoneClient");
    	}   	
        --x;
        if (color == 553648127) {
            color = 16777215;
